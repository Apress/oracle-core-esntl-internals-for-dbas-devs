rem
rem	Script:		snap_9_latch_child.sql
...
create or replace package snap_latch_child as

	procedure start_snap(i_latch in varchar2);
	procedure end_snap(i_latch in varchar2, i_all_sleeps in boolean default false);

	procedure start_snap_cbc;
	procedure end_snap_cbc(i_all_sleeps in boolean default false);

end;
/

create or replace package body snap_latch_child as

	cursor c1 (i_latch in varchar2) is
		select
			la.addr,
			la.child#	indx,
			la.gets,
...
procedure start_snap (i_latch in varchar2)  is
begin
...
procedure end_snap(i_latch in varchar2, i_all_sleeps in boolean default false) is

	m_latch_count	number(6) := 0;
begin
	...
	dbms_output.put_line(i_latch || ' latch waits - ' || 
	...
	for r in c1(i_latch) loop
		...
		if (
			...
			m_latch_count := m_latch_count + 1;

			dbms_output.put(rpad(r.addr,17));
	...
	dbms_output.put_line('Latches reported: ' || m_latch_count);

end end_snap;

procedure start_snap_cbc is
begin
	start_snap('cache buffers chains');
end;

procedure end_snap_cbc(i_all_sleeps in boolean default false) is
begin
	end_snap('cache buffers chains', i_all_sleeps);
end;
...
end snap_latch_child;
/


drop public synonym snap_latch_child;
create public synonym snap_latch_child for snap_latch_child;
grant execute on snap_latch_child to public;
